...
export class MakeOfferModalComponent implements OnInit {
  ...
  constructor(
    ...
    this.currentUserId = this.jwtDecoderService.decodedToken?.id;;
    ...
    this.bidForm = this.fb.group({
      bid_price: ['', [Validators.required, Validators.pattern('^[0-9]*$')]],
    });
    this.bidForm = this.fb.group({
      bid_price: [
        '',
  ...
  bidCheckValidator(control: any) {
    ...
    return control.value >=
      this.calculatePercentageIncrease(
        this.highBid || this.product.auction_final_price,
        1
      )
      ? null
      : { 'bid-check': true };
